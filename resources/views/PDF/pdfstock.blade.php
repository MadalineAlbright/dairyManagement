 ...
 <span style="color: red; border:1px solid red; border-radius: 10% ;width: 200px;
	height: 200px; font-size: 19px; ">UNPAID</span>
      ...
      <table style="padding-top: 100px;">
        ...
        @if($Invoice[0]->statusPaid==0)
          <tr>
            <td class="sum" colspan="5" style="text-align: left;">SUBTOTAL</td>
            <td class="total"> {{$Invoice[0]->price}} </td>
          </tr>
          <tr>
              <td  class="sum" colspan="5" style="text-align: left;">PAID</td>
              <td class="total"> {{$paymenthist}}</td>
          </tr>
          <tr>
              <td colspan="5" class="grand total" style="text-align: left;"> TOTAL</td>
              <td class="grand total"> {{$Invoice[0]->price-$paymenthist}}</td>
          </tr>
          <tr>
            <td  class="sum" colspan="5" style="text-align: left;">Previous Due</td>
            <td class="total"> {{($Invoice[0]->supplyer->total_balance-$Invoice[0]->supplyer->paid)}}</td>
          </tr>
          <tr>
            <td colspan="5" class="grand total" style="text-align: left;">GRAND TOTAL</td>
            <td class="grand total"  style="text-align: right"> {{($Invoice[0]->supplyer->total_balance-$Invoice[0]->supplyer->paid)}}</td>
          </tr>
        @else
          <tr>
